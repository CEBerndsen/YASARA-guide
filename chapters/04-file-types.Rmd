...
Once you have your model or scene in a form that is useful, you can also export the data. This is the one action I typically prefer to use the GUI over the command line, however I will show the command line method as well (For example: \@ref(fig:savepdb)). Additionally, saving via the GUI has one trick, so I will show you how to do this in pictures as well (For example: \@ref(fig:savepdbmenu)). 
...
plot_grid(sa, sap, sam1, sam2, labels = c("File", "Save As", "Select Obj(s)", "Select Filename and Destination"), nrow =  2, align = "both", axis = c("l", "t"))
